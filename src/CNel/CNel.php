...
*/
  public function ThemeEngineRender() {
    ...
    $themePath = NEL_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = NEL_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $nel->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $nel
    ...
    $nel = &$this;
    // First the default Nel themes/functions.php
    include(NEL_INSTALL_PATH . '/themes/functions.php');
    ...
    // Extract $nel->data to own variables and handover to the template file
...
public function CreateUrl($urlOrController=null, $method=null, $arguments=null) {
  ...
  /**
* Draw HTML for a menu defined in $nel->config['menus'].
*
